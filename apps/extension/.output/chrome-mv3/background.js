var _background = function() {
  ...
  const definition = defineBackground(() => {
    ...
    browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (message.action === "SAVE_JOB") {
        saveJob(message.jobData).then((response) => sendResponse({ success: true, data: response })).catch((err) => sendResponse({ success: false, error: err.message }));
        return true;
      }
    });
  });
  async function saveJob(jobData) {
    ...
    if (!res.ok) {
      ...
      throw new Error(((_a = errorData == null ? void 0 : errorData.error) == null ? void 0 : _a.message) || `Failed to save job (${res.status})`);
    }
    return res.json();
  }
  _background;
