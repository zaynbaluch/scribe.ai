...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-[1280px] mx-auto space-y-8 pb-12">
      {/* Onboarding Section */}
      <section className="relative overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface-transparent)] p-8 mb-8">
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--bg-elevated)] border border-[var(--border-subtle)] text-xs font-medium text-[var(--text-secondary)] mb-4">
            <TrendingUp size={14} /> Getting Started
          </div>
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight mb-2 text-[var(--text-primary)]">Welcome to Scribe.ai</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="flex gap-4">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-[var(--bg-elevated)] border border-[var(--border-subtle)] flex items-center justify-center font-bold text-sm text-[var(--text-primary)]">1</div>
              <div>
                <h4 className="font-semibold mb-1 text-[var(--text-primary)]">Finish your profile</h4>
                <p className="text-sm text-[var(--text-muted)] mb-3">Add your experience, skills, and projects to train your AI.</p>
                <Link href="/profile" className="text-xs font-semibold text-[var(--text-primary)] hover:opacity-70 transition-opacity">Complete Profile →</Link>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-[var(--bg-elevated)] border border-[var(--border-subtle)] flex items-center justify-center font-bold text-sm text-[var(--text-primary)]">2</div>
              <div>
                <h4 className="font-semibold mb-1 text-[var(--text-primary)]">Create a base resume</h4>
                <p className="text-sm text-[var(--text-muted)] mb-3">Build a master resume from your profile as a starting point.</p>
                <Link href="/resumes" className="text-xs font-semibold text-[var(--text-primary)] hover:opacity-70 transition-opacity">Create Resume →</Link>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-[var(--bg-elevated)] border border-[var(--border-subtle)] flex items-center justify-center font-bold text-sm text-[var(--text-primary)]">3</div>
              <div>
                <h4 className="font-semibold mb-1 text-[var(--text-primary)]">Tailor & Apply</h4>
                <p className="text-sm text-[var(--text-muted)] mb-3">Paste a job description and let the AI tailor your story.</p>
                <Link href="/tailor" className="text-xs font-semibold text-[var(--text-primary)] hover:opacity-70 transition-opacity">Start Tailoring →</Link>
              </div>
            </div>
          </div>
        </div>
