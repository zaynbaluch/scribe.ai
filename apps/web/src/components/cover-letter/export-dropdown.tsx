...
import { Download, FileText, FileType, Type, Loader2, ChevronDown } from 'lucide-react';
...
const FORMATS = [
  ...
  { id: 'txt', label: 'Plain Text', icon: Type, desc: 'Raw text file' },
];

export default function ExportDropdown({ id }: ExportDropdownProps) {
  ...
  const doExport = async (format: string) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/cover-letters/${id}/export/${format}`, {
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-50 w-52 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] shadow-xl py-1 animate-in fade-in slide-in-from-top-2 duration-150">
            {FORMATS.map((f) => (
              <button key={f.id} onClick={() => doExport(f.id)}
